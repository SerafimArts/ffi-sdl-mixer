...
const INPUT_HEADERS = __DIR__ . '/SDL_mixer.h';
...
$result = (new PhpStormMetadataGenerator(
        argumentSetPrefix: 'ffi_sdl_mixer_',
        ignoreDirectories: ['/usr', ...explode(',', $_SERVER['FFI_GENERATOR_IGNORE_PATHS'] ?? '')],
        naming: new class(
            entrypoint: Serafim\SDL\Mixer\Mixer::class,
            externalNamespace: 'Serafim\SDL\Mixer',
        ) extends SimpleNamingStrategy {
            protected function getEnumTypeName(string $name): string
            {
                return match ($name) {
                    'MIX_InitFlags' => \Serafim\SDL\Mixer\InitFlags::class,
                    'Mix_Fading' => \Serafim\SDL\Mixer\Fading::class,
                    'Mix_MusicType' => \Serafim\SDL\Mixer\MusicType::class,
